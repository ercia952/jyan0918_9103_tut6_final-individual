...
// set the random seed for random values and noise patterns
  randomSeed(10);
  noiseSeed(20);
...
class Circle {
  ...
  update() {
    // smooth the random motion with perlin noise
    ...
    this.angle += this.rotationSpeed; // set the angle
  }
...
class Line {
  constructor(x1, y1, x2, y2, color, weight) {
    ...
    this.liftSpeed = random(0.00005, 0.000002); // set the speed for upward movement
  }
  ...
  update() {
    ...
    this.y2 = this.initialY2 + (noise(this.offsetY2 + frameCount * 0.01